...
function EquipmentDetailPage() {
  ...
  if (data.kind === "exclusive") {
    return <ExclusiveEquipmentDetailScaffold data={data} />;
  }
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-md bg-muted px-2 py-1 text-xs font-semibold text-muted-foreground">
                  {classification.groupKo} · {classification.subtypeKo}
                </span>
                <span className="rounded-md bg-muted px-2 py-1 text-xs font-semibold text-muted-foreground">
                  {tabLabel}
                </span>
              ...
              {identity.nameKr === null ? (
                <p className="mt-2 text-sm text-muted-foreground">
                  한국명이 아직 검수 확정되지 않아 중문명 <span className="font-medium text-foreground">{identity.nameCn}</span>을 임시 표시하고 있어.
                </p>
              ) : (
                <p className="mt-2 text-sm text-muted-foreground">중문명 {identity.nameCn}</p>
              )}
            </div>

            <div className="rounded-xl border border-border bg-muted/35 px-4 py-3 text-sm text-muted-foreground">
              equipmentId <span className="font-semibold text-foreground">{data.equipmentId}</span>
            </div>
          </div>
        </header>
...
function ExclusiveEquipmentDetailScaffold({ data }: { data: ExclusiveEquipmentDetailPageData }) {
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      ...
      <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
        <p className="text-sm text-muted-foreground">Equipment Stage 4-4 예정</p>
        <h1 className="mt-1 text-3xl font-bold text-foreground">{data.displayName}</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          equipmentId {data.equipmentId} · 전용 영웅 {data.ownerHero.nameKr}
        </p>
        <p className="mt-5 text-sm leading-6 text-muted-foreground">
          전용장비 상세 표현은 Stage 4-4에서 전용 목록과 함께 완성해.
        </p>
      </section>
    </main>
  );
