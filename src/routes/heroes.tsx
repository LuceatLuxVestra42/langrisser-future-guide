  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
        <Link
          to="/"
          className="sticky top-3 z-40 inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-primary/25 bg-card px-4 py-2.5 text-sm font-semibold text-foreground shadow-sm transition hover:border-primary/45 hover:bg-accent"
        >
        ...
        <section aria-label="영웅 목록 필터" className="mt-4 rounded-lg border border-border bg-card p-3 sm:p-4">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <span className="mb-1.5 block text-[14px] font-bold text-foreground">통합 필터</span>
          ...
          <div className="mt-4 border-t border-border pt-4">
            <label htmlFor="hero-search" className="mb-1.5 block text-[14px] font-bold text-foreground">
              이름 검색
            </label>
...
function FilterSectionButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onClick}
      className={`h-10 whitespace-nowrap rounded-md border px-4 text-[14px] font-bold transition ${
        active
          ? "border-foreground bg-foreground text-background"
          : "border-border bg-background text-foreground hover:border-foreground/30"
      }`}
