...
export const Route = createFileRoute("/heroes/$heroId")({
  ...
  head: ({ loaderData }) => ({ meta: [{ title: loaderData ? `${loaderData.hero.identity.nameKr ?? loaderData.hero.identity.nameCn} | 랑그릿사 모바일 영웅` : "영웅 | 랑그릿사 모바일 미래시 정보" }] }),
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="mx-auto w-full max-w-6xl px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        <Link to="/heroes" className="inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground transition hover:text-foreground"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> 영웅 목록</Link>
        <section className="mt-5 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="grid gap-0 md:grid-cols-[220px_minmax(0,1fr)]">
            <div className="aspect-[4/5] bg-muted/40 md:aspect-auto md:min-h-[275px]">
              {imageUrl ? <img src={imageUrl} alt="" className="h-full w-full object-cover object-top" /> : <div className="flex h-full min-h-[250px] flex-col items-center justify-center gap-3 text-muted-foreground"><UserRound className="h-16 w-16" strokeWidth={1.15} aria-hidden="true" /><span className="inline-flex items-center gap-1 text-xs font-semibold"><ImageOff className="h-3.5 w-3.5" aria-hidden="true" />이미지 연결 대기</span></div>}
            </div>
            <div className="p-5 sm:p-7">
              <div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-foreground px-2 py-1 text-xs font-bold text-background">{hero.rarity.baseLabel}</span>{detail.systems.spReleased ? <span className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-2 py-1 text-xs font-bold text-foreground"><Sparkles className="h-3.5 w-3.5" aria-hidden="true" />SP</span> : null}</div>
              <h1 className="mt-4 text-3xl font-bold tracking-tight text-foreground sm:text-4xl">{displayName}</h1>
              <div className="mt-2 space-y-0.5 text-sm text-muted-foreground"><p>중문명 {hero.identity.nameCn}</p>{hero.identity.nameEn ? <p>영문명 {hero.identity.nameEn}</p> : null}<p>Hero ID {hero.heroId}</p></div>
        ...
        <section className="mt-5 rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">{detail.jobs.branches.map((branch) => <div key={branch.branchIndex} className="rounded-xl border border-border bg-muted/20 p-4"><div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-foreground"><span>분기 {branch.branchIndex}</span><span className="text-muted-foreground">·</span><span>{branch.jobs.map((job) => job.nameCn ?? `Job ${job.jobId ?? "?"}`).join(" → ") || "직업 정보 없음"}</span></div>{branch.capstone ? <div className="mt-4"><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>최종 직업 {branch.capstone.nameCn ?? branch.capstone.jobId ?? "-"}</span>{branch.capstone.heroLevel != null ? <span>Lv.{branch.capstone.heroLevel}</span> : null}{branch.capstone.star != null ? <span>{branch.capstone.star}성 기준</span> : null}{branch.capstone.statStatus ? <span>{branch.capstone.statStatus}</span> : null}</div><StatGrid stats={branch.capstone.finalStats} /></div> : null}</div>)}</div>
        </section>
        <section className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)]">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6"><SectionTitle icon={<ShieldCheck className="h-4 w-4" aria-hidden="true" />} title="확정 시스템 연결" /><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><StatusChip label="유대" value={`${detail.systems.bondRowCount}개`} /><StatusChip label="전용장비" value={detail.systems.exclusiveEquipmentStatus ?? "-"} /><StatusChip label="중앙율정" value={detail.systems.centralDisciplineStatus ?? "-"} /><StatusChip label="SP" value={detail.systems.spStatus ?? "-"} /><StatusChip label="스킨" value={`${detail.presentation.skinCount}개`} /></div></div>
        </section>
        <section className="mt-5 rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6"><SectionTitle icon={<Database className="h-4 w-4" aria-hidden="true" />} title="상세 데이터 상태" /><p className="mt-3 text-sm leading-6 text-muted-foreground">이 페이지는 FINAL_FROZEN Stage 6 전체 묶음을 읽지 않고 현재 Hero의 개별 shard 하나만 읽어 표시용 데이터로 투영해. 원본 ConfigData 관계 재계산이나 이름·ID 추론은 하지 않아.</p><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-md bg-muted px-2 py-1 font-semibold text-foreground">Stage 6 · {stage6.admissionStatus}</span><span className="rounded-md bg-muted px-2 py-1 font-semibold text-foreground">구조 {detail.validation.structuralStatus}</span><span className="rounded-md bg-muted px-2 py-1 font-semibold text-foreground">게시 {detail.validation.publicationStatus ?? "-"}</span><span className="rounded-md bg-muted px-2 py-1 font-semibold text-foreground">review {detail.validation.reviewCount}</span></div></section>
      </div>
    </main>
  );
}
...
function StatusChip({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-2 rounded-lg border border-border bg-muted/20 px-3 py-2"><span className="font-semibold text-foreground">{label}</span><span className="text-muted-foreground">{value}</span></div>; }
