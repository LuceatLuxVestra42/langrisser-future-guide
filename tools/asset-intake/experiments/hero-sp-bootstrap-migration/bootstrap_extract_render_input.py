...
CONTRACT = ROOT / 'contract' / 'hero-sp.v1.json'
if not CONTRACT.is_file():
    raise SystemExit('FAIL_MISSING_ASSET_CONTRACT')

HERO_ID=int(os.environ['HERO_ID'])
CHAR_IMAGE_ID=int(os.environ['CHAR_IMAGE_ID'])
OUTPUT_ROOT=pathlib.Path(os.environ.get('OUTPUT_ROOT','generated/work'))
UA='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/140 Safari/537.36'
ev=json.loads(CONTRACT.read_text(encoding='utf-8'))
rows=[r for r in ev['records'] if int(r['heroId'])==HERO_ID and int(r['charImageId'])==CHAR_IMAGE_ID]
if len(rows)!=1: raise RuntimeError(f'contract record cardinality {len(rows)}')
r=rows[0]
ver=ev['source']['officialInstallerVersion']
base=f'http://mhmnzupdate.zlongame.com/MHMNZ/InstallVersion/InstallPage_{ver}'
out=OUTPUT_ROOT/str(HERO_ID)
rawdir=out/'raw'
...
url=f"{base}/{r['packageName']}"
n=head(url)
entries=zip_dir(url,n)
bm=[e for e in entries if norm(e['name'])==norm(r['bundleEntry'])]
if len(bm)!=1:raise RuntimeError(f'exact bundle entry count {len(bm)}')
bundle=fetch_entry(url,bm[0])
bsha=hashlib.sha256(bundle).hexdigest()
if bsha!=r['bundleSha256']:
    raise SystemExit(f"FAIL_SOURCE_HASH_MISMATCH heroId={HERO_ID} expected={r['bundleSha256']} actual={bsha}")
...
runtime='assets/gameproject/runtimeassets/'+norm(r['sourceSpinePath'])
pm=[(p,o) for p,o in containers if norm(p)==runtime]
...
for _ in range(3):
    ...
    for o in queue:
        ...
        seen.add(pid);typ=o.type.name;nm=name_of(o)
        if typ=='TextAsset':
...
atlas=rawdir/f'{apid}-{aname}.atlas.txt';skel=rawdir/f'{spid}-{sname}.skel.bytes'
atlas.write_bytes(abytes);skel.write_bytes(sbytes)
...
page=lines[0].strip();texruntime=norm(str(pathlib.PurePosixPath(r['sourceSpinePath']).parent/page))
tm=[(p,o) for p,o in containers if norm(p)==('assets/gameproject/runtimeassets/'+texruntime)]
