...
verify("QA2_REFERENCE_PREDECESSOR_PROOF", () => {
  ...
  assert(contract.predecessor?.contract === "data/contracts/project-doctor-route-hosted-qa-stage2.v1.json", "QA-2 predecessor contract path mismatch");
  assert(/^[0-9a-f]{40}$/.test(contract.predecessor?.proofHeadSha ?? ""), "QA-2 reference proof SHA must be a full commit SHA");
  assert(Number.isInteger(contract.predecessor?.proofRunId) && contract.predecessor.proofRunId > 0, "QA-2 reference proof run must be recorded");
  assert(contract.predecessor?.proofConclusion === "success", "QA-2 reference proof conclusion mismatch");
});
...
verify("DEEP_HISTORY_COVERAGE", () => {
  ...
  assert(spec.includes('name: "레온 상세 보기"'), "current Hero detail accessible-name assertion missing");
...
const result = {
  ...
  referencePredecessorProof: contract.predecessor,
  mainlineRuntimeProof: "REGENERATE_AFTER_INTEGRATION",
...
if (check) {
  if (!fs.existsSync(outputPath)) {
    throw new Error(`Missing mainline validation artifact: ${path.relative(root, outputPath)}. Run with --write after package integration.`);
  }
  const current = readJson(outputPath);
  assert(
    JSON.stringify(current) === JSON.stringify(result),
    "Mainline QA-3 validation artifact is stale or structurally non-deterministic",
  );
}
