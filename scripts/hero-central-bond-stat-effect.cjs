...
function parseHeartFetterLv10Flat(description) {
  ...
  if (!match) throw new Error(`unsupported HeartFetter Lv10 flat description: ${normalized}`);
  return Object.freeze({ hp: Number(match[1]), df: Number(match[2]), magicDf: Number(match[3]) });
}
