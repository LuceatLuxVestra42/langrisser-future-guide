...
import * as stage5 from './lib/configdata-lookup-stage5.mjs';

async function main() {
  execFileSync(process.execPath, ['scripts/validate-configdata-lookup-stage5.mjs'], { stdio: 'inherit' });
  ...
  const selected = await (await import('./lib/configdata-lookup-stage2.mjs')).loadSourceTypes(selectedContract);
  ...
  console.log('PASS_CONFIGDATA_LOOKUP_STAGE6_INCREMENTAL_REBUILD');
}
