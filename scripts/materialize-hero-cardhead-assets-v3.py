...
for bundle_name in sorted(by_bundle):
    ...
    container_rows = [
        (normalize_path(container_path), str(container_path), obj)
    ...
    for row in by_bundle[bundle_name]:
        ...
        sprite_hits = [hit for hit in path_hits if object_type(hit[2]) == "Sprite"]
        if len(sprite_hits) != 1:
            path_types = [
                {
                    "containerPath": original,
                    "objectType": object_type(obj),
                    "pathId": int(getattr(obj, "path_id", 0)),
                }
                for _, original, obj in path_hits
            ]
            raise RuntimeError(
                f"Hero {row['heroId']} exact Sprite container mismatch in {bundle_name}: "
                f"source={row['sourceCardHeadPath']} pathHits={len(path_hits)} spriteHits={len(sprite_hits)} pathTypes={path_types}"
            )

        _, container_path, obj = sprite_hits[0]
        data = obj.read()
        image = getattr(data, "image", None)
        if image is None:
            raise RuntimeError(f"Hero {row['heroId']} Sprite has no image: {container_path}")
        ...
        row["spritePathId"] = int(getattr(obj, "path_id", 0))
        row["spriteName"] = getattr(data, "m_Name", None)
        row["exactPathObjectTypes"] = sorted(object_type(hit[2]) for hit in path_hits)
...
if len({(r["sourceBundleName"], r["spritePathId"]) for r in materialized}) != 267:
    raise RuntimeError("resolved Sprite identity uniqueness drift")
...
manifest = {
    ...
    "gameVersion": VER,
    "sourceContract": "ConfigDataHeroInfo.CharImage_ID -> ConfigDataCharImageInfo.ID -> CardHeadImage -> exact Unity container path -> unique Sprite object",
...
validation = {
    ...
    "distinctSpriteIdentityCount": len({(r["sourceBundleName"], r["spritePathId"]) for r in materialized}),
    ...
    "rule": "Exact ConfigData CardHeadImage path -> exact updater bundle family -> exact Unity container path -> unique Sprite object. No name JOIN, filename similarity, ID arithmetic, _0/_1 suffix guessing, or Hero semantic recomputation.",
}
MANIFEST.parent.mkdir(parents=True, exist_ok=True)
VALIDATION.parent.mkdir(parents=True, exist_ok=True)
MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
VALIDATION.write_text(json.dumps(validation, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(json.dumps({"status": validation["status"], "materializedCount": len(materialized), "fileCount": file_count, "representative": validation["representative"], "bundleReports": bundle_reports}, ensure_ascii=False, indent=2))
