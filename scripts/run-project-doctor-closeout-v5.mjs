...
export const CLOSEOUT_STEPS_V5 = [
...
export const executeCloseoutV5 = ({ argv = [], executor = defaultExecutor } = {}) => {
  const executions = [];
  for (const step of CLOSEOUT_STEPS_V5) {
  ...
  return { status: 'PASS_PROJECT_DOCTOR_CLOSEOUT_V5', exitCode: 0, executions };
};
...
if (isMain) {
  if (process.argv.includes('--help') || process.argv.includes('-h')) {
    ...
    console.log('Runs D1 status, D5 freshness checks, D4 V5 self-test, then the D3 V5-selected D4 V5 plan.');
    process.exit(0);
  }
  const result = executeCloseoutV5({ argv: process.argv.slice(2) });
