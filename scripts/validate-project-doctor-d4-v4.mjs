...
const d3Contract=JSON.parse(fs.readFileSync('data/contracts/project-doctor-d3-validator-plan.v4.json','utf8'));
const d4Contract=JSON.parse(fs.readFileSync('data/contracts/project-doctor-d4-execution.v4.json','utf8'));
const catalog=d3Contract.checkCatalog;
const byId=id=>catalog.find(x=>x.id===id);
const check=id=>({...byId(id),execution:'PLANNED'});
const plan=(status,ids=[],manualReviews=[])=>({version:4,stage:'D3',status,changedFileCount:status==='NO_CHANGES'?0:1,selectedChecks:ids.map(check),manualReviews});
const fixtures=[];
...
runFixture('DRY_RUN_ZERO_EXEC',()=>{let calls=0;const r=executePlan({plan:plan('PLAN_READY',['coverage-hero-canonical','equipment-image-final','skin-assets-final','localization-audit','production-build']),d3Contract,d4Contract,dryRun:true,executor:()=>{calls++;return{status:0}}});assert.equal(r.status,'PASS_DRY_RUN');assert.equal(calls,0)});
...
runFixture('SKIN_ASSET_OWNER_EXECUTES',()=>{const seen=[];const r=executePlan({plan:plan('PLAN_READY',['skin-assets-final','production-build']),d3Contract,d4Contract,executor:item=>{seen.push(item.id);return{status:0}}});assert.equal(r.status,'PASS_EXECUTED');assert.deepEqual(seen,['skin-assets-final','production-build'])});
runFixture('PHASE_ORDER',()=>{const seen=[];const ids=['doctor-health-gate','production-build','localization-audit','skin-assets-final','regression-coverage-promotion-v2','equipment-image-final','coverage-hero-canonical','configdata-integrity'];const r=executePlan({plan:plan('PLAN_READY',ids),d3Contract,d4Contract,executor:item=>{seen.push(item.id);return{status:0}}});assert.equal(r.status,'PASS_EXECUTED');assert.deepEqual(seen,['configdata-integrity','coverage-hero-canonical','equipment-image-final','regression-coverage-promotion-v2','skin-assets-final','localization-audit','production-build','doctor-health-gate'])});
...
runFixture('SKIN_ASSET_FAIL_FAST',()=>{const seen=[];const r=executePlan({plan:plan('PLAN_READY',['skin-assets-final','localization-audit','production-build']),d3Contract,d4Contract,executor:item=>{seen.push(item.id);return{status:item.id==='skin-assets-final'?6:0}}});assert.equal(r.status,'FAIL_CHECK');assert.equal(r.failedCheckId,'skin-assets-final');assert.deepEqual(seen,['skin-assets-final'])});
...
runFixture('TAMPER_BLOCKS_ALL',()=>{let calls=0;const p=plan('PLAN_READY',['skin-assets-final']);p.selectedChecks[0].command='npm run evil';const r=executePlan({plan:p,d3Contract,d4Contract,executor:()=>{calls++;return{status:0}}});assert.equal(r.status,'INVALID_PLAN');assert.equal(calls,0)});
runFixture('UNKNOWN_CHECK_BLOCKS_ALL',()=>{let calls=0;const p=plan('PLAN_READY');p.selectedChecks=[{id:'unknown',phase:3,command:'npm run validate:skin-assets-final'}];const r=executePlan({plan:p,d3Contract,d4Contract,executor:()=>{calls++;return{status:0}}});assert.equal(r.status,'INVALID_PLAN');assert.equal(calls,0)});
...
runFixture('STRICT_NPM_ALIAS_SHAPE',()=>{const p=plan('PLAN_READY',['skin-assets-final']);p.selectedChecks[0].command='node scripts/validate-skin-assets-final.mjs';const r=executePlan({plan:p,d3Contract,d4Contract,executor:()=>({status:0})});assert.equal(r.status,'INVALID_PLAN')});

assert.equal(d4Contract.schemaId,'project-doctor-d4-execution/v4');
...
console.log(JSON.stringify({status:'PASS_PROJECT_DOCTOR_D4_V4_EXECUTION_FIXTURES',fixturePassCount:fixtures.length,fixtureCount:fixtures.length,fixtures,allowedCheckCount:d4Contract.allowedCheckIds.length,actualRepositoryCommandExecutionCount:0},null,2));
