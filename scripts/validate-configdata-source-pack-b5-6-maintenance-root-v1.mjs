...
function validateResolverBoundary() {
  ...
  const stage1Producer = read('scripts/finalize-soldier-training-tech-classification-stage1.mjs');
  requireIncludes(stage1Producer, "resolveConfigDataFile('ConfigDataTrainingTechInfo.json')", 'TrainingTech Stage1 producer');
  requireIncludes(stage1Producer, "resolveConfigDataFile('ConfigDataTrainingTechLevelInfo.json')", 'TrainingTech Stage1 producer');
  requireIncludes(stage1Producer, 'data/configdata/ConfigDataTrainingTechInfo.json', 'TrainingTech Stage1 producer logical provenance');
  requireIncludes(stage1Producer, 'data/configdata/ConfigDataTrainingTechLevelInfo.json', 'TrainingTech Stage1 producer logical provenance');

  const stage1Validator = read('scripts/validate-soldier-training-tech-classification-stage1.mjs');
  requireIncludes(stage1Validator, "resolveConfigDataFile('ConfigDataTrainingTechInfo.json')", 'TrainingTech Stage1 validator');
  requireIncludes(stage1Validator, "resolveConfigDataFile('ConfigDataTrainingTechLevelInfo.json')", 'TrainingTech Stage1 validator');

  const stage2Validator = read('scripts/validate-soldier-training-tech-classification-stage2.mjs');
  requireIncludes(stage2Validator, "path.startsWith('data/configdata/')", 'TrainingTech Stage2 validator physical path boundary');
  requireIncludes(stage2Validator, 'resolveConfigDataFile', 'TrainingTech Stage2 validator');
}
...
function main() {
  ...
  const covered = new Set((contract.coveredReaders || []).map((entry) => entry.path));
  for (const required of [
    'scripts/lib/configdata-direct.cjs',
    'scripts/build_shared_movement_types.mjs',
    'scripts/analyze-equipment-acquisition.mjs',
    'scripts/investigate_hero_display_stats.cjs',
    'scripts/finalize-soldier-training-tech-classification-stage1.mjs',
    'scripts/validate-soldier-training-tech-classification-stage1.mjs',
    'scripts/validate-soldier-training-tech-classification-stage2.mjs',
  ]) {
    if (!covered.has(required)) fail('B5.6 covered reader set drift', required);
  }
  ...
  console.log(JSON.stringify({
    ...
    coveredReaderCount: covered.size,
