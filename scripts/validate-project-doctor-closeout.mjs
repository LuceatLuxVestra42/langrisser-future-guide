...
runFixture('ALL_STEPS_ORDERED', () => {
  ...
  assert.deepEqual(seen.map(item => item[0]), ['d1-status', 'd4-self-test', 'd4-run']);
...
runFixture('D1_FAILURE_STOPS', () => {
  const seen = [];
  const result = executeCloseout({ executor: step => { seen.push(step.id); return { status: step.id === 'd1-status' ? 2 : 0 }; } });
...
runFixture('D4_SELF_TEST_FAILURE_STOPS', () => {
  ...
  assert.deepEqual(seen, ['d1-status', 'd4-self-test']);
});
