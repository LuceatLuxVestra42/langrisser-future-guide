...
runFixture('ALL_STEPS_ORDERED', () => {
  ...
  assert.deepEqual(seen.map(item => item[0]), ['d5-fresh-status', 'd5-self-test', 'd4-self-test', 'd4-run']);
  assert.deepEqual(seen[0][1], []);
  assert.deepEqual(seen[3][1], ['--dry-run']);
});

runFixture('D5_REFRESH_FAILURE_STOPS', () => {
  const seen = [];
...
runFixture('D5_SELF_TEST_FAILURE_STOPS', () => {
  ...
  assert.deepEqual(seen, ['d5-fresh-status', 'd5-self-test']);
});
...
runFixture('D4_SELF_TEST_FAILURE_STOPS', () => {
  ...
  assert.deepEqual(seen, ['d5-fresh-status', 'd5-self-test', 'd4-self-test']);
});
