...
def run_scan(pkg, verifier):
    import UnityPy
    targets={verifier.norm(x):x for x in REVIEW_PATHS}
    hits={x:[] for x in REVIEW_PATHS}; scan_errors=[]; bundle_count=0; package_count=0
    for p in pkg['packages']:
        package_count+=1
        try: entries=verifier.zip_directory(p['url'],p['contentLength'])
        except Exception as e:
            scan_errors.append({'packagePart':p['part'],'reason':f'PACKAGE_CATALOG_INCOMPLETE:{type(e).__name__}:{e}'}); continue
        for e in entries:
            ...
            try:
                ...
                scan_errors.append({'packagePart':p['part'],'bundleEntry':e['name'],'reason':f'BUNDLE_OR_UNITY_DECODE_FAIL:{type(ex).__name__}:{ex}'}); continue
            for container_path,value in env.container.items():
                ...
                try:
                    ...
                    row={'packagePart':p['part'],'packageName':p['packageName'],'bundleEntry':e['name'],'bundleSha256':verifier.sha256_bytes(raw),'runtimeContainerPath':str(container_path).replace('\\','/'),'objectType':typ}
                    if typ=='Sprite':
                    ...
                    scan_errors.append({'packagePart':p['part'],'bundleEntry':e['name'],'runtimeContainerPath':str(container_path),'reason':f'EXACT_HIT_DECODE_FAIL:{type(ex).__name__}:{ex}'})
    return package_count,bundle_count,hits,scan_errors
...
def build():
    ...
    result={'version':1,'schemaId':'hero-awakening-icon-source-snapshot-gap/v1','status':'BLOCKER' if blockers else 'PASS','completion':'INCOMPLETE' if blockers else 'COMPLETE','semanticReopen':False,'stage':'A6_7_SOURCE_SNAPSHOT_GAP_CLASSIFICATION','predecessor':{'stage':'A6_6_BULK_256_VERIFICATION','commit':'9e3d4050e463885b65666e5c8efebc27176217aa','bulkResultSha256':EXPECTED_BULK},'source':{'kind':'OFFICIAL_INSTALLER','installVersion':'1.1.113','packageInventorySha256':EXPECTED_PKG,'searchCoverage':'EXHAUSTIVE_ALL_BUNDLES'},'summary':{'reviewInputCount':3,'packageScanCount':package_count,'bundleScanCount':bundle_count,'verifiedCount':sum(r['status']=='VERIFIED' for r in results),'notInSourceSnapshotCount':sum(r['status']=='NOT_IN_SOURCE_SNAPSHOT' for r in results),'blockerCount':blockers,'scanErrorCount':len(errors)},'results':results,'scanErrors':errors,'nextStage':'A6_8_NEWER_OFFICIAL_SOURCE_SNAPSHOT_IF_NEEDED'}
