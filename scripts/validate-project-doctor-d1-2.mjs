...
const classifyProjectHealth = (healthCounts, precedence) => {
  for (const health of precedence) {
    if ((healthCounts[health] ?? 0) > 0) return health;
  }
  return 'UNKNOWN';
};
...
for (const record of actualDomains) {
  const spec = contract.domains?.[record.domain];
  ...
  for (const [key, expected] of Object.entries(expectedPopulation)) {
    ...
    if (hasOwn(record.population ?? {}, key) && record.population[key] !== expected) {
      populationMismatches.push({ key, expected, actual: record.population[key] });
    }
  }
  const result = {
    ...
    pass: Boolean(spec)
      ...
      && record.primarySource?.path === spec.primaryStatusSource
    ...
    primarySourceMatchesContract: record.primarySource?.path === spec?.primaryStatusSource,
...
const summary = {
  ...
  nextStageGoal: gatePass ? 'Wire the read-only collector/health gate into a stable CLI entry point without adding GitHub Actions yet.' : null,
};
