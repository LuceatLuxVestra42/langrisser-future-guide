...
runFixture('DRY_RUN_ZERO_EXEC',()=>{let calls=0;const r=executePlan({plan:plan('PLAN_READY',['coverage-hero-canonical','localization-audit','production-build']),d3Contract,d4Contract,dryRun:true,executor:()=>{calls++;return{status:0}}});assert.equal(r.status,'PASS_DRY_RUN');assert.equal(calls,0)});
...
runFixture('TAMPER_BLOCKS_ALL',()=>{let calls=0;const p=plan('PLAN_READY',['coverage-hero-canonical']);p.selectedChecks[0].command='npm run evil';const r=executePlan({plan:p,d3Contract,d4Contract,executor:()=>{calls++;return{status:0}}});assert.equal(r.status,'INVALID_PLAN');assert.equal(calls,0)});
...
runFixture('STRICT_NPM_ALIAS_SHAPE',()=>{const p=plan('PLAN_READY',['coverage-hero-canonical']);p.selectedChecks[0].command='node scripts/validate_hero_stage4_final.cjs';const r=executePlan({plan:p,d3Contract,d4Contract,executor:()=>({status:0})});assert.equal(r.status,'INVALID_PLAN')});
...
console.log(JSON.stringify({status:'PASS_PROJECT_DOCTOR_D4_V3_EXECUTION_FIXTURES',fixturePassCount:fixtures.length,fixtureCount:10,fixtures,allowedCheckCount:d4Contract.allowedCheckIds.length,actualRepositoryCommandExecutionCount:0},null,2));
