...
let frozenSheetEvidence = null;
try {
  const probe = read('data/validation/hero-page-stage5-2-exclusive-sheet-probe.json');
  const row3 = probe.sampleRows?.find(x => x.row === 3) ?? null;
  frozenSheetEvidence = row3 ? {
    status: probe.status,
    row: row3.row,
    equipmentNameKr: row3.cells?.[2] ?? null,
    ownerNameKr: row3.cells?.[3] ?? null,
    expectedEquipmentId: 273,
    resolvedSkillHero: Number(equipmentById.get(273)?.SkillHero ?? 0),
    resolvedCanonicalOwnerNameKr: canonicalById.get(Number(equipmentById.get(273)?.SkillHero ?? 0))?.nameKr ?? null
  } : null;
} catch {}
...
const validation = {
  ...
  ownershipEvidence: {
    ...
    frozenKoreanSheetCrossCheck: frozenSheetEvidence,
    rejectedFallbacks: [
