...
const v3=JSON.parse(fs.readFileSync('data/contracts/project-doctor-d3-validator-plan.v3.json','utf8'));
...
const expectedAdmitted=[...promotionV2Owners,'equipment-assets','skin-assets'];
const expectedManual=['hero-assets','banner-assets'];
...
for(const node of expectedManual){if(v3.manualReviewNodes?.[node]===undefined)failures.push({type:'MANUAL_SCOPE_NOT_FROM_V3',node});}
...
if((contract.checkCatalog??[]).find(x=>x.id==='skin-assets-final')?.command!=='npm run validate:skin-assets-final')failures.push({type:'SKIN_ASSET_OWNER_CATALOG_MISMATCH'});
...
const skin=contract.skinAssetOwnerPromotion;
if(skin?.node!=='skin-assets'||skin?.phase!==3||skin?.canonicalSkinCount!==540||skin?.canonicalHeroCount!==267||skin?.heroesWithSkinCount!==235||skin?.zeroSkinHeroCount!==32||skin?.publicSkinPngCount!==540||skin?.verifiedPublicHashCount!==540||skin?.browserUi!=='PASS_SKIN_STAGE3_6_BROWSER_UI'||skin?.browserUiFreshness!=='PASS_FRESH_BROWSER_UI_EVIDENCE'||skin?.semanticStageReopened!==false||skin?.sourceOrderRecomputed!==false||skin?.liveBrowserExecutionInsideDoctor!==false||!/^[0-9a-f]{64}$/.test(skin?.browserInputFingerprint??'')||!/^[0-9a-f]{40}$/.test(skin?.deployedSourceSha??''))failures.push({type:'SKIN_ASSET_PROMOTION_EVIDENCE_MISMATCH',evidence:skin});
const pass=failures.length===0;
const summary={version:4,schemaId:'project-doctor-d3-summary/v4',stage:'D3',checkpoint:'PROJECT_DOCTOR_D3_VALIDATOR_PLAN_V4_SKIN_ASSET_OWNER_ADMISSION',status:pass?'PASS_PROJECT_DOCTOR_D3_PLAN_V4':'FAIL_PROJECT_DOCTOR_D3_PLAN_V4',completion:pass?'COMPLETE':'BLOCKED',contract:CONTRACT_PATH,checks:{selectionFixtureCount:fixtureResults.length,selectionFixturePassCount:fixtureResults.filter(x=>x.pass).length,verifiedCatalogEntryCount:contract.checkCatalog.length,admittedOwnerCount:contract.admittedOwners.length,manualReviewNodeCount:Object.keys(contract.manualReviewNodes).length},fixtureResults,failures,hardErrorCount:failures.length};
