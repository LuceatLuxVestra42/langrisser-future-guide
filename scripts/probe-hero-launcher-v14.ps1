...
def imm_xrefs(va):
 ...
 while True:
  ...
  if pos<0: break
  raw=text['rawPointer']+pos; outx.append({'codeRaw':raw,'codeVa':text_raw_to_va(raw),'nearestPrologueVa':(text_raw_to_va(find_prologue(raw)) if find_prologue(raw) is not None else None)})
  start=pos+1
 return outx
def call_xrefs(destva):
 hits=[]
 rp=text['rawPointer']; end=rp+text['rawSize']-5
 for raw in range(rp,end):
  if b[raw]!=0xE8: continue
  rel=struct.unpack_from('<i',b,raw+1)[0]; src=text_raw_to_va(raw); dest=src+5+rel
  if dest==destva: hits.append({'callRaw':raw,'callVa':src,'nearestPrologueVa':(text_raw_to_va(find_prologue(raw)) if find_prologue(raw) is not None else None)})
 return hits
# raw offsets confirmed in v13 string map
targets={
 'RunStart':0x3131F0,
 'FetchServerVersionFailed':0x313268,
 'BuildPatchChainFailed':0x3132C8,
 'PatchListStep':0x31345C,
 'AutoPatchPrefix':0x313498,
 'PatchInfoFormat':0x3135D8,
 'RunDone':0x313678,
 'InstallFetchServerVersion':0x313918,
 'InstallRemoteLocal':0x313970,
 'ConcatFormat':0x3139DC,
 'InstallInfoFormat':0x3139E8,
 'UpdateURI':0x311704,
}
rows=[]
for name,raw in targets.items():
 rva=raw_to_rva(raw); va=imagebase+rva
 rows.append({'target':name,'raw':raw,'va':va,'xrefs':imm_xrefs(va)})
helpers=[0x449E50,0x449EE0,0x449F10,0x43CD60,0x440780,0x417D00,0x49BB50,0x49F260]
helperRows=[{'helperVa':va,'callers':call_xrefs(va)} for va in helpers]
# collect candidate function starts from debug-string xrefs and helper callers around patch-flow area
starts=set([0x449E50,0x449EE0,0x449F10,0x43CD60,0x440780])
for r in rows:
 for x in r['xrefs']:
...
# keep manageable windows: 4096 bytes from each start, capped in .text
wins=[]
for va in sorted(starts):
 raw=va_to_raw(va)
 if raw is None or not(text['rawPointer']<=raw<text['rawPointer']+text['rawSize']): continue
 n=min(4096,text['rawPointer']+text['rawSize']-raw)
...
print(json.dumps({'status':summary['status'],'windowCount':len(wins),'rows':[{ 'target':r['target'],'xrefs':len(r['xrefs'])} for r in rows]},ensure_ascii=True))
...
Get-Content (Join-Path $report 'v14-callers.txt') -Raw|Write-Host
