import { evaluateResolutionQa } from './validate-skin-stage3-3-3-resolution-qa.mjs';
...
function makeScan() {
    ...
    candidateResults: [
      {
        ...
        matches: [{ embeddedCab, runtimePathByteOffset: 10 + index }],
      },
    ],
  ...
  return {
    ...
    source: { bundleFilenameCatalog: { lineCount: 2, uniqueNameCount: 2, sha256: 'catalog-sha' } },
    counts: {
      frozenSkinCount: 2,
...
const base = makeScan();
let result = evaluateResolutionQa(base, contract);
add(
  'SINGLE_EXACT_FINAL_PASS',
  result.finalFreezeReady && result.status === 'PASS_SKIN_STAGE3_3_3_RESOLUTION_QA_FREEZE_READY',
);
...
result = evaluateResolutionQa(alias, contract);
...
result = evaluateResolutionQa(conflict, contract);
add(
  'NONIDENTICAL_CAB_REVIEW',
  !result.finalFreezeReady &&
    result.status === 'REVIEW_SKIN_STAGE3_3_3_RESOLUTION_QA' &&
    result.counts.reviewRequiredTargetCount === 1,
);
...
result = evaluateResolutionQa(pending, contract);
add(
  'UNSCANNED_REMAINS_PENDING',
  !result.finalFreezeReady &&
    result.status === 'WAITING_FOR_STAGE3_3_2_FULL_SCAN' &&
    result.counts.unscannedCandidateBundleCount === 1,
);
...
result = evaluateResolutionQa(missing, contract);
add(
  'ALL_CANDIDATES_NOT_FOUND_FAILS',
  !result.finalFreezeReady && result.status === 'FAIL_SKIN_STAGE3_3_3_RESOLUTION_QA',
);
...
result = evaluateResolutionQa(duplicate, contract);
add('DUPLICATE_OCCURRENCE_REVIEW', !result.finalFreezeReady && result.counts.reviewRequiredTargetCount === 1);

let tamperBlocked = false;
try {
  const tamper = structuredClone(base);
  tamper.counts.requiredTargetCount = 5;
  evaluateResolutionQa(tamper, contract);
} catch {
  tamperBlocked = true;
}
add('FROZEN_COUNT_TAMPER_BLOCKED', tamperBlocked);

const failed = checks.filter((check) => !check.pass);
const summary = {
  status:
    failed.length === 0
      ? 'PASS_SKIN_STAGE3_3_3_RESOLUTION_QA_SELFTEST'
      : 'FAIL_SKIN_STAGE3_3_3_RESOLUTION_QA_SELFTEST',
