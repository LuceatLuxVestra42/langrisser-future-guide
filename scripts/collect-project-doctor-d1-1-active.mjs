...
const normalizeReview = (value, sourcePath, selector, index = null) => {
  if (typeof value === 'string') return { code: value, scope: 'unknown', source: sourcePath, selector, raw: value };
  if (!isObject(value)) return { code: null, scope: 'unknown', source: sourcePath, selector, raw: value };
  return {
...
const lifecycleFromRules = (source, rules = []) => {
  for (const rule of rules) if (conditionMatches(source, rule.when)) return rule.lifecycle;
  return 'UNKNOWN';
};
...
const expectedValueForKey = (expectedKey, primarySelected, supplements) => {
  ...
  for (const supplement of supplements) if (supplement.selected[expectedKey] !== undefined) return supplement.selected[expectedKey];
...
const domainRecord = (domain, spec, activeMeta) => {
  const sourcePath = spec.primaryStatusSource;
  const primaryRead = safeReadJson(sourcePath);
  if (!primaryRead.ok) {
    return {
      domain, lifecycle: 'UNKNOWN', health: 'MISSING', rawStatus: null, rawCompletion: null, rawFreezeState: null,
      population: {}, hardErrorCount: null, reviewCount: 0, reviews: [], blockers: [], facets: [],
      primarySource: { path: sourcePath, readError: primaryRead.error }, supplementalSources: [], nextWork: [],
      activeSource: activeMeta, notes: ['Primary status source could not be read.'],
    };
  }
  ...
  for (const supplementalSpec of spec.supplementalSources ?? []) {
    ...
    if (!read.ok) {
      ...
      supplements.push({ path: supplementalSpec.path, role: supplementalSpec.role, facet: supplementalSpec.facet, selected: {}, readError: read.error, policy: supplementalSpec.policy ?? null, supersedesPrimaryReviewCodes: supplementalSpec.supersedesPrimaryReviewCodes ?? [] });
      continue;
    }
    const selected = selectValues(read.data, supplementalSpec.selectors);
    supplements.push({ path: supplementalSpec.path, role: supplementalSpec.role, facet: supplementalSpec.facet, selected, missingSelectors: selectorMissing(selected, supplementalSpec.selectors), policy: supplementalSpec.policy ?? null, supersedesPrimaryReviewCodes: supplementalSpec.supersedesPrimaryReviewCodes ?? [] });
  }

  let reviews = [];
  for (const selector of spec.reviewSelectors ?? []) reviews.push(...reviewsFromSelector(jsonPointer(primary, selector), sourcePath, selector));
  ...
  reviews = reviews.filter(review => !(review.code && supersededCodes.has(review.code))).map(review => ({ ...review, scope: reviewScope(review) }));

  const blockers = [];
  if (typeof primarySelected.blocker === 'string' && primarySelected.blocker.length > 0) blockers.push({ source: sourcePath, code: primarySelected.blocker });
  ...
  for (const [key, value] of Object.entries(primarySelected)) if (/failedCheckCount/i.test(key) && typeof value === 'number' && value > 0) explicitFailureSignals.push(key);
  for (const supplement of supplements) {
    for (const [key, value] of Object.entries(supplement.selected ?? {})) if (/failedCheckCount/i.test(key) && typeof value === 'number' && value > 0) explicitFailureSignals.push(`${supplement.role}.${key}`);
  }
  ...
  const statusLooksAccepted = typeof rawStatus === 'string' && (rawStatus.toUpperCase().startsWith('PASS') || rawStatus === 'READY_FOR_ASSET_EVIDENCE');
  let health = 'UNKNOWN';
  ...
  const facets = [
    sourceSnapshot(sourcePath, 'PRIMARY_STATUS_SOURCE', spec.primaryFacet, primarySelected),
    ...supplements.map(item => ({ ...sourceSnapshot(item.path, item.role, item.facet, item.selected, item.policy), ...(item.readError ? { readError: item.readError } : {}) })),
  ];

  return {
    domain, lifecycle, health, rawStatus, rawCompletion, rawFreezeState,
    population: populationFromExpected(spec.expected, primarySelected, supplements),
    hardErrorCount, reviewCount: reviews.length, reviews, blockers, facets,
    primarySource: sourceSnapshot(sourcePath, 'PRIMARY_STATUS_SOURCE', spec.primaryFacet, primarySelected),
    supplementalSources: supplements.map(item => ({ ...sourceSnapshot(item.path, item.role, item.facet, item.selected, item.policy), ...(item.readError ? { readError: item.readError } : {}) })),
    nextWork: (spec.nextWorkSelectors ?? []).map(selector => ({ source: sourcePath, selector, value: jsonPointer(primary, selector) })),
...
const { result: registry } = buildRegistry({ write: true });
if (registry.status !== 'PASS_PROJECT_DOCTOR_ACTIVE_SOURCE_REGISTRY') {
  throw new Error(`Active Source Registry blocked: ${registry.status} (${registry.hardErrorCount} errors)`);
}

const domains = contract.scope.domains.map(domain => {
  const spec = registry.effectiveDomains?.[domain];
  const activeMeta = registry.domains?.[domain];
  if (!spec || !activeMeta) throw new Error(`Missing active source registry domain: ${domain}`);
  return domainRecord(domain, spec, activeMeta);
});
...
const projectHealth = precedence.find(value => healthCounts[value] > 0) ?? 'UNKNOWN';
...
const sourceReadIssueCount = domains.reduce((sum, item) => sum + (item.primarySource?.readError ? 1 : 0) + item.supplementalSources.filter(source => source.readError).length, 0);

const output = {
  version: 1, schemaId: 'project-doctor-d1-1-status/v1', stage: 'D1-1', status: 'COLLECTED', contract: CONTRACT_PATH,
  activeSourceRegistry: REGISTRY_PATH, readOnly: true, validatorExecutionCount: 0, rawConfigDataReadCount: 0, semanticRecomputationCount: 0,
  projectHealth, healthCounts, lifecycleCounts, knownHardErrorTotal, reviewTotal, blockerTotal, domains,
};

const collectorPass = domains.length === 6;
const summary = {
  version: 1, stage: 'D1-1', checkpoint: 'PROJECT_DOCTOR_D1_1_COLLECTOR',
  status: collectorPass ? 'PASS_PROJECT_DOCTOR_D1_1_COLLECTOR' : 'FAIL_PROJECT_DOCTOR_D1_1_COLLECTOR',
  completion: 'COMPLETE', contract: CONTRACT_PATH, activeSourceRegistry: REGISTRY_PATH, output: OUTPUT_PATH, projectHealth,
  checks: {
    contractFrozen: contract.status === 'DESIGN_FROZEN',
    activeSourceRegistryPass: registry.status === 'PASS_PROJECT_DOCTOR_ACTIVE_SOURCE_REGISTRY',
    activeSourceSelectedCount: registry.selectedCount,
    domainCount: { expected: 6, actual: domains.length, pass: domains.length === 6 },
    missingDomainCount: healthCounts.MISSING, inconsistentDomainCount: healthCounts.INCONSISTENT, failedDomainCount: healthCounts.FAIL,
    sourceReadIssueCount, validatorExecutionCount: 0, rawConfigDataReadCount: 0, semanticRecomputationCount: 0, productionDataWriteCount: 0,
  },
  healthCounts, lifecycleCounts, knownHardErrorTotal, reviewTotal, blockerTotal,
  domainStates: Object.fromEntries(domains.map(item => [item.domain, {
    lifecycle: item.lifecycle, health: item.health, rawStatus: item.rawStatus, population: item.population,
    hardErrorCount: item.hardErrorCount, reviewCount: item.reviewCount, blockerCount: item.blockers.length,
    activeSourceId: item.activeSource?.selectedId ?? null, activeSourcePath: item.primarySource?.path ?? null,
  }])),
  nextStage: null,
};
