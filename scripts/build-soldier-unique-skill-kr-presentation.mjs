...
assert(overrideById.size === 21, `expected 21 review overrides, got ${overrideById.size}`);
...
assert(passCount === 130, `PASS count must be 130, got ${passCount}`);
assert(reviewCount === 55, `REVIEW count must be 55, got ${reviewCount}`);
