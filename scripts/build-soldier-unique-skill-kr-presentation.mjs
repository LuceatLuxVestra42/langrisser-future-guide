...
assert(overrideById.size === 28, `expected 28 review overrides, got ${overrideById.size}`);
...
assert(passCount === 137, `PASS count must be 137, got ${passCount}`);
assert(reviewCount === 48, `REVIEW count must be 48, got ${reviewCount}`);
