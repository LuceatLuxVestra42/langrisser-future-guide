...
const { composeLegacyValues } = require('./hero-stage5-legacy-stat-composer.cjs');
const {
  CENTRAL_BOND_RATE,
...
const leonLegacy = composeLegacyValues({
  statInputs: leonStatInputs,
  masteryFlats: { hp:452, at:73, magic:0, df:10, magicDf:10, dex:5 },
  normalBondRate: 0.25,
  spBonusRates: { hp:0.05, df:0.05 },
});
assert.deepStrictEqual(leonLegacy.values, { hp:4041, at:602, magic:224, df:260, magicDf:231, dex:125 });

const leonFlat = parseHeartFetterLv10Flat('英雄的生命<color=#DC143C>+750</color>，防御<color=#DC143C>+30</color>，魔防<color=#DC143C>+40</color>。');
const leon = applyCentralBondToLegacy({ legacyValues: leonLegacy.values, statInputs: leonStatInputs, flat: leonFlat });
...
assert.strictEqual(Math.round(leonDfRaw * 1.30) - Math.round(leonDfRaw * 1.25), 9);
assert.strictEqual(computeCentralBondPercentDelta({ini:53,up:8,starCorrection:7796}), 10);
...
assert.throws(() => applyCentralBondToLegacy({ legacyValues: {}, statInputs: leonStatInputs, flat: leonFlat }), /legacyValues\.hp/);

console.log(JSON.stringify({
  status: 'PASS',
  checkpoint: 'HERO_CENTRAL_BOND_STAT_EFFECT_STAGE5',
  sourceDerivedFixtureCount: sourceDerivedFixtures.length,
  leonLegacyValues: leonLegacy.values,
  leonPercentDeltas: leon.percentDeltas,
