...
try {
  ...
  if (!['plan', 'doctor', 'run'].includes(command)) {
    throw new Error('Usage: node scripts/configdata-lookup-stage8.mjs <plan|doctor|run> [--changed PATH ... | --base SHA --head SHA] [--json] [--github-output FILE]');
  }
  const inputs = loadStage8Inputs();
  const plan = selectProjectDoctorExpansion(resolveChangedPaths(options), inputs);
  ...
  if (command === 'run') {
    ...
    process.stdout.write(`${JSON.stringify(runSelectedExpansion(plan, inputs), null, 2)}\n`);
