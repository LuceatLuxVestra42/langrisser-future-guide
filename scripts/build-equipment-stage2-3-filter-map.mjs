...
for(const r of equipment.filter(canonical)){
  ...
  records.push({id:r.ID,name:r.Name,group:cls.group,groupKo:ko[cls.group],subtype:cls.subtype,subtypeKo:ko[cls.subtype],basis:cls.basis});
}
const result={
...
console.log(JSON.stringify({canonicalCount:result.canonicalCount,counts:result.counts,unclassified:result.unclassified.length},null,2));
