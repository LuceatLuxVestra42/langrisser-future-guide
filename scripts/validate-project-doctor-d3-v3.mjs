...
const expectedAdmitted=['hero-canonical','soldier-canonical','equipment-canonical','hero-soldier-relation','hero-equipment-relation','banner-data','skin-relation','shared-movement','soldier-assets'];
const expectedManual=['hero-assets','equipment-assets','banner-assets','skin-assets'];
...
for(const row of contract.admittedOwners??[]){
  ...
  if(!catalog||!catalog.triggerNodes?.includes(row.node)||catalog.command!==`npm run ${row.packageCommand}`||catalog.phase!==2) failures.push({type:'OWNER_CATALOG_MISMATCH',row,catalog});
}
...
const summary={version:3,schemaId:'project-doctor-d3-summary/v3',stage:'D3',checkpoint:'PROJECT_DOCTOR_D3_VALIDATOR_PLAN_V3_REGRESSION_ADMISSION',status:pass?'PASS_PROJECT_DOCTOR_D3_PLAN_V3':'FAIL_PROJECT_DOCTOR_D3_PLAN_V3',completion:pass?'COMPLETE':'BLOCKED',contract:CONTRACT_PATH,checks:{selectionFixtureCount:fixtureResults.length,selectionFixturePassCount:fixtureResults.filter(x=>x.pass).length,verifiedCatalogEntryCount:contract.checkCatalog.length,admittedOwnerCount:contract.admittedOwners.length,manualReviewNodeCount:Object.keys(contract.manualReviewNodes).length},fixtureResults,failures,hardErrorCount:failures.length};
