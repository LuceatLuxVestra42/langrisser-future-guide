...
const CONTRACT_PATH = 'data/contracts/project-doctor-d4-execution.v2.json';
const argv = process.argv.slice(2);
if (argv.includes('--help') || argv.includes('-h')) {
  console.log('Usage: node scripts/run-project-doctor-d4-v2.mjs [--dry-run] [--json] [D3 changed-file source options]');
...
try {
  ...
  if (options.json) console.log(JSON.stringify(output, null, 2));
    ...
    console.log('PROJECT DOCTOR EXECUTION — D4 V2');
  ...
  if (options?.json) console.log(JSON.stringify({ stage: 'D4-V2', status: 'INVALID_PLAN', error: message }, null, 2));
  else console.error(`[doctor:run:v2] ${message}`);
  process.exitCode = 2;
}
