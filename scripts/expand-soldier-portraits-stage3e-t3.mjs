...
const EXTERNAL_SOURCES = new Map([
  [250, {
    admission: 'CURRENT_KR_EXACT_WITH_ASSOCIATED_IMAGE',
    ...
    assetNeedle: '%EC%A0%95%EA%B8%80%20%EC%82%AC%EC%8A%AC%20%EB%AC%B4%ED%9D%AC.webp',
  }],
  [341, {
    admission: 'CURRENT_KR_EXACT_WITH_ASSOCIATED_IMAGE',
    ...
    assetNeedle: 'miturin.png',
  }],
  [422, {
    admission: 'CURRENT_ABILITY_ALIAS_WITH_ASSOCIATED_IMAGE',
    ...
    assetNeedle: '20_1_%EB%93%9C%EB%9E%98%EA%B3%A0%EB%8B%88%EC%95%84%EB%82%98%EC%9D%B4%ED%8A%B8.webp',
    abilityNeedles: ['6%', '30%', '이동력 +2'],
  }],
  [647, {
    admission: 'CURRENT_KR_EXACT_WITH_ASSOCIATED_IMAGE',
  ...
  [1036, {
    ...
    assetNeedle: '21_3_%EC%97%B0%EC%98%A5%EC%9D%98%EB%A7%88%EB%A0%A5%EC%BD%94%EC%96%B4.webp',
  }],
]);
...
function associated(html, name, assetNeedle) {
  const idx = html.indexOf(name);
  if (idx < 0) return false;
  const chunk = html.slice(Math.max(0, idx - 2500), idx + 1200);
  return chunk.includes(assetNeedle) || chunk.includes(decodeURIComponent(assetNeedle));
}
...
// A. Stage 3C unique legacy ability matches: pinned GitHub WebP source.
for (const soldierId of [249, 515, 517, 645]) {
  ...
  const record = {
    soldierId,
    nameKr: canonical.identity?.nameKr ?? d.nameKr ?? null,
    tier: 3,
    sourceKind: 'PINNED_LEGACY_KR_WEBP_STAGE3C_UNIQUE_ABILITY',
    sourceUrl: d.legacySourceUrl,
    sourceName: d.legacyName,
    fileName,
    resolutionMethod: 'SAME_TIER_ARMY_UNIQUE_LEGACY_ABILITY_NUMERIC_PROGRESSION',
    size: bytes.length,
    sha256: sha256(bytes),
    width: null,
    height: null,
  ...
  evidenceRows.push({ soldierId, status: 'PASS', admission: 'STAGE3C_UNIQUE_LEGACY_ABILITY', evidenceName: d.legacyName, evidenceUrl: d.legacySourceUrl, assetUrl: d.legacySourceUrl });
}

// B. Current Korean-name/ability web evidence with an image associated in the same recommendation row.
for (const [soldierId, source] of EXTERNAL_SOURCES) {
  ...
  const html = await fetchText(source.evidenceUrl);
  if (!associated(html, source.evidenceName, source.assetNeedle)) throw new Error(`Evidence page does not associate image and name for ${soldierId}`);

  if (source.admission === 'CURRENT_KR_EXACT_WITH_ASSOCIATED_IMAGE') {
  ...
  } else if (source.admission === 'CURRENT_ABILITY_ALIAS_WITH_ASSOCIATED_IMAGE') {
    for (const needle of source.abilityNeedles ?? []) {
      if (!html.includes(needle)) throw new Error(`Ability evidence needle missing for ${soldierId}: ${needle}`);
    }
  } else {
    throw new Error(`Unknown Stage 3E admission ${source.admission}`);
  }
  ...
  const record = {
    soldierId,
    nameKr: canonicalName,
    tier: 3,
    sourceKind: source.admission === 'CURRENT_KR_EXACT_WITH_ASSOCIATED_IMAGE'
      ? 'CURRENT_KR_WEB_EVIDENCE_ASSOCIATED_IMAGE'
      : 'CURRENT_ABILITY_ALIAS_WEB_EVIDENCE_ASSOCIATED_IMAGE',
    sourceUrl: source.assetUrl,
    evidenceUrl: source.evidenceUrl,
    evidenceName: source.evidenceName,
    fileName,
    resolutionMethod: source.admission,
    size: bytes.length,
    sha256: sha256(bytes),
  ...
  newlyResolved.push(record);
  unresolvedById.delete(soldierId);
  evidenceRows.push({ soldierId, status: 'PASS', admission: source.admission, canonicalName, evidenceName: source.evidenceName, evidenceUrl: source.evidenceUrl, assetUrl: source.assetUrl });
}
...
const output = {
  version: 4,
  stage: 'frontend-stage3e-t3-evidence-backed-expansion',
  ...
  policy: {
    ...v3.policy,
    stage3eLegacyAdmission: 'Reuse only Stage 3C PASS_UNIQUE_LEGACY_ABILITY_MATCH pinned WebP sources.',
    stage3eCurrentWebAdmission: 'Require canonical Korean exact name and image association on one evidence page, except alias case 422 which additionally requires matching ability evidence.',
  ...
  sources: {
    ...
    stage3cDiscovery: DISCOVERY_PATH,
    currentCanonical: CURRENT_PATH,
    stage3eEvidence: EVIDENCE_OUTPUT,
  },
  coverage: {
    canonicalSoldierCount: 224,
    canonicalNormalCount: current.filter((r) => !r.identity.isSp).length,
    canonicalSpCount: current.filter((r) => r.identity.isSp).length,
    resolvedCount: records.length,
    unresolvedCount: unresolved.length,
    resolvedNormalCount: records.filter((r) => !isSp(r.soldierId)).length,
    resolvedSpCount: records.filter((r) => isSp(r.soldierId)).length,
    tier1Resolved: tierCount(1),
    tier2Resolved: tierCount(2),
    tier3Resolved: tierCount(3),
