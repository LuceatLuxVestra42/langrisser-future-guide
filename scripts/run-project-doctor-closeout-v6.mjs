...
export const CLOSEOUT_STEPS_V6 = [
  ...
  { id: 'd4-self-test', script: 'scripts/validate-project-doctor-d4-v6.mjs', passthrough: false },
  { id: 'd4-run', script: 'scripts/run-project-doctor-d4-v6.mjs', passthrough: true },
];

const defaultExecutor = (step, args) => spawnSync(process.execPath, [step.script, ...args], { stdio: 'inherit', shell: false });

export const executeCloseoutV6 = ({ argv = [], executor = defaultExecutor } = {}) => {
  const executions = [];
  for (const step of CLOSEOUT_STEPS_V6) {
  ...
  return { status: 'PASS_PROJECT_DOCTOR_CLOSEOUT_V6', exitCode: 0, executions };
};
...
const result = executeCloseoutV6({ argv: process.argv.slice(2) });
